...
async function insertTestData() {
  try {
    ...
    const user3Result = await pool.query(
      "SELECT id FROM users WHERE username = 'bob_johnson' LIMIT 1",
    );
    const user4Result = await pool.query(
      "SELECT id FROM users WHERE username = 'alice_williams' LIMIT 1",
    );
    const user5Result = await pool.query(
      "SELECT id FROM users WHERE username = 'mike_brown' LIMIT 1",
    );
    ...
    const conv3Result = await pool.query(
      'INSERT INTO conversations DEFAULT VALUES RETURNING id',
    );
    ...
    console.log('Conversations created with IDs:', {
      conv1Id,
      conv2Id,
      conv3Id,
    });
